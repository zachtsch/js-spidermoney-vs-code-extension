...
## Windows
Downloads the SpiderMonkey shell from the pinned Firefox 140.13.0 ESR release, verifies the ZIP file against Mozilla's published SHA-256 checksum, and unzips it in `C:\spidermonkey`. Then it updates your **user** `Path` so future terminals can find `js`. Finally it removes the `.zip` file.
